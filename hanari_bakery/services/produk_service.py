...
class ProfitCalculator(ProfitCalculatorInterface):
    def hitung_profit(self, product: ProdukRoti, jumlah_pcs: int) -> Dict:
        ...
        # Hitung sisa roti yang belum terjual
        ...
        return {
            ...
            # Data Kondisi Riil Sesuai Permintaan (Untuk Tambahan di Bawah)
            "pendapatan_riil": pendapatan_permintaan,
            "profit_riil": profit_colored(profit_permintaan),
...
def profit_colored(nilai: float) -> str:
    """Helper untuk memberi tanda format rupiah positif atau negatif"""
    if nilai < 0:
...
class ProductService:
    """
    Service untuk mengelola operasi bisnis produk.
    KINI 100% MEMATUHI OCP: Registry kelas produk disuntikkan dari luar.
    """
    def __init__(
        self,
    ...
    def tambah_produk(self, kode: str) -> Tuple[bool, str]:
        ...
        produk_class = self._product_registry[kode]
        produk = produk_class()
    ...
    def tambah_produk_custom(
        ...
        produk = ProdukCustom(
            ...
            biaya_produksi_per_n=biaya,
            harga_jual_per_n=harga,
    ...
    # ---- Simulasi Produksi & Logika Bisnis ----
    def simulasi_produksi(self, kode: str, jumlah_pcs: int = 0) -> Optional[Dict]:
        ...
        jumlah_resep = 1
        if jumlah_pcs > 0:
            jumlah_resep = math.ceil(jumlah_pcs / resep_pack)
        ...
        # --- PERBAIKAN LOG HISTORY: Ambil data riil dari profit_info ---
        ...
        self._history.simpan(history_record)
    ...
    def estimasi_profit(self, kode: str, jumlah_pcs: int):
        """Helper jembatan agar UI Menu bisa membaca kalkulasi tanpa efek samping ke stok & history"""
        # 1. Cari produk berdasarkan kode ke repository katalog produk
        produk = self._repo.find_by_code(kode)
        if not produk:
            return None
            
        # 2. Langsung hitung menggunakan objek calculator
        data_hasil = self._calc.hitung_profit(produk, jumlah_pcs)
        
        # --- TAMBAHKAN LOGIKA HITUNG BEP MINIMAL JUAL DI SINI ---
        resep = produk.jumlah_per_resep
        harga_per_pcs = produk.harga_jual_per_n / resep
        total_biaya = data_hasil["total_biaya"]
        
        # Cari tahu butuh berapa pcs total terjual agar pendapatannya >= total_biaya
        total_pcs_butuh_bep = math.ceil(total_biaya / harga_per_pcs) if harga_per_pcs > 0 else 0
        
        # Pcs tambahan yang harus dijual lagi dari jumlah permintaan saat ini
        pcs_tambahan_lagi = total_pcs_butuh_bep - jumlah_pcs
        if pcs_tambahan_lagi < 0:
            pcs_tambahan_lagi = 0
            
        # Masukkan variabel baru ke dalam dictionary hasil
        data_hasil["pcs_tambahan_lagi"] = pcs_tambahan_lagi
